...
funcdeskbar <- function(data) {
  ...
  legend("topright", levels(data$treatstatus),
    fill = global_kicols[1:4],
    col = global_kicols[1:4], border = global_kicols[1:4], bty = "n",
    cex = cexmy
  )
}
```
...
```{r descmref, cache=cacheon, dependson="bardesctreat"}
mref <- funcdesk2(ef2 = "HFmrEF")

default_kable(mref %>% select(treat, treatstatus, nper),
  col.names = c("Treatment", "Status", "n (%)"),
  caption = "Treatment status after first HFH - HFmrEF",
  escape = TRUE,
  scale = F
)
```

```{r descmrefbar, fig.cap = "Treatment status after first HFH - HFmrEF", cache=cacheon, dependson=c("bardesctreat", "descmref"), fig.width=9, fig.height=7}
...
default_kable(all %>% select(treat, treatstatus, nper),
  col.names = c("Treatment", "Status", "n (%)"),
  caption = "Treatment status after first HFH - All (HFrEF + HFmrEF)",
  escape = TRUE,
  scale = F
)
```

```{r descallbar, fig.cap = "Treatment status after first HFH - All (HFrEF + HFmrEF)", cache=cacheon, dependson=c("bardesctreat", "descall"), fig.width=9, fig.height=7}
